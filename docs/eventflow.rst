...
이벤트 라우터의 룰은 이벤트의 흐름을 결정하는데, 패턴과 컬렉터로 구성된다. 패턴은 앞에서 소개한 이벤트 태그의 패턴을 말하고, 컬렉터는 교정 또는 출력 플러그인을 말한다.
...
파이프라인은 선택적으로 하나 이상의 교정 플러그인으로 시작하고, 출력 플러그인으로 끝난다.
...
5. 파이프라인은 이벤트를 등록된 교정 플러그인을 거쳐 출력 플러그인의 ``emit_events`` 메소드를 통해 그것으로 보낸다.
