...
개별(separate) 스레드 모델은 입력에서 출력 플러그인까지의 모든 플러그인이 같은 스레드 아래에서 수행되는 모델이다. 출력 플러그인이 하나의 입력 플러그인에서 생성되는 이벤트만 다룰 때 사용된다.
...
.. code-block:: yaml
    ...
    sources:
      - i.filetail -f file1 | o.file -f out1  # 입력 스레드 1에서 실행
      - i.filetail -f file2 | o.file -f out2  # 입력 스레드 2에서 실행


``file1`` 과 ``file2`` 을 위해 모두 두 개의 입력 스레드가 할당된다.


결합 스레드 모델
^^^^^^^^^^^^^^^^

결합(aggregated) 스레드 모델은 여러 입력 플러그인에서 생성한 이벤트가 하나의 출력 플러그인으로 모여서 처리가 되는 경우이다. 이 경우 각 입력 플러그인과 출력 플러그인을 위해 하나씩 스레드가 생성된다.
...
``file1`` 과 ``file2`` 의 입력을 위해 두 개, 그리고 ``file`` 에 결합된 출력을 위해 한 개, 모두 세 개의 스레드가 생성된다.
...
.. code-block:: yaml
    ...
    sources:
      - i.counter | m.reform -w tag t1 | tag mytag1
      - i.counter | m.reform -w tag t2 | tag mytag2

    matches:
      mytag*: o.stdout

``m.reform`` 은 입력 스레드 1, 2에서 각각 실행되고, 이 결과가 출력 스레드로 전달된다. 그러나 다음과 같은 경우는:

.. code-block:: yaml

    sources:
      - i.counter | tag mytag1
      - i.counter | tag mytag2

    matches:
      mytag*: m.reform -w tag all | o.stdout

두 입력 스레드에서 생성된 레코드에 대해 출력 스레드에서 ``m.reform`` 이 실행된다.
...
#. 데이터 라우터와 개별 플러그인 생성.
#. 데이터 라우터에 ``add_rule`` 로 플러그인 추가
#. 데이터 라우터의 ``build_threads`` 메소드를 통해 스레드 객체 생성
#. 각 스레드 객체에 대해 ``run`` 호출
...
#. 각 입력 플러그인을 위한 스레드(**입력 스레드**) 생성. 이때 데이터 라우터를 복사(라우터의 플러그인들은 아직 리소스가 생성되지 않은 상태)
...
- 각 스레드는 데이터 라우터를 깊은 복사해 가짐.
- 데이터 라우터의 출력 플러그인은 각각 두 개의 출력 프락시로 변경된다.
...
.. important:: 스레드가 생성될 때 데이터 라우터에 있는 파이프라인에 있는 플러그인들을 **복사** 한다. 따라서 모든 플러그인들은 복사가 **가능** 해야 한다. 일반적인 파이썬 객체는 ``copy.copy`` 에 의해 복사가 가능하나, 만약 플러그인 내에서 직접 C/C++를 통해  메모리 할당을 하는 등의 경우라면 특별 메소드 ``__copy__`` 와 ``__deepcopy__`` 를 구현해 주어야 할 것이다.
...
3. 출력 스레드 동작
  ...
  #. 스레드 메인에서 주기적으로 ``read_queue`` 하고, 얻어진 데이터 스트림으로 ``handle_stream`` 호출
...
결합 스레드 모델에서 출력이 늦어지는 경우 동작 예
-------------------------------------------------

출력이 입력 속도를 따르지 못하는 경우 딜레이가 생기게 된다. 이 경우를 ``i.filetail`` 의 예로 설명하겠다.

1. ``i.filetail`` 은 입력 스레드에서 대상 파일을 모니터링(watch) 하다가,
2. 파일이 변하면 정해진 파일 읽기 위치(``read_pos``)부터 블럭 크기로 IO 한 번에 읽어들인다.
3. 읽어들인 행들을 행 버퍼에 넣고, 2의 과정을 반복한다. 이때 메모리를 너무 많이 차지하지 않게 정해진 행수 이상이면 중단한다. 행 버퍼에 행에 맞게 파일 읽기 위치(``read_pos``)를 설정하고, 다음으로 넘어간다.
4. 읽어들인 행 버퍼를 필요한 처리 후, 큐에 ``put`` 을 시도한다.
 - 이때 큐가 차지 않았으면 바로 완료될 것이다.
 - 만약 큐가 차 있으면 입력 스레드는 여기서 block 상태가 된다. 출력 스레드에서 큐의 기존 데이터를 처리하여야 block이 풀리고, 다음 행을 ``put`` 할 수 있게 된다.
5. 모든 행을 ``put`` 하였으면
 - 버퍼가 ``MemoryBuffer`` 인 경우 출력이 완료되면 ``read_pos`` 가 갱신된다.
 - 버퍼가 ``FileBuffer`` 인 경우 바로 ``read_pos`` 가 갱신된다.

.. note:: ``i.filetail`` 의 경우 얼마나 대상 파일이 처리되었는지 기록해두는 것이 중요하다. ``i.filetail`` 은 이 정보를 ``read_pos`` 에 설정하고, POS 파일에 저장한다.

결합 스레드 모델에서 파싱이 매치 스레드에서 수행되는 경우
---------------------------------------------------------
...
.. code-block:: yaml
    ...
    matches:
      log: p.syslog | o.stdout

이 경우 소스 스레드에서는 파싱되지 않은 텍스트 상태로 ``DataStream`` 을 구성하고 이것을 큐를 통해 매칭 스레드로 넘겨 준다. 매칭 스레드에서는 이것을 파싱하여 출력한다.
