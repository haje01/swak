...
플러그인은 크게 입력, 파서, 교정, 버퍼, 출력 그리고 명령 플러그인의 여섯가지 타입으로 나뉜다. 각 타입별 플러그인은 다음과 같은 패키지명 형식으로 시작한다.
...
- 교정: 라인을 필터링 하거나, 레코드의 특정 필드를 추가, 삭제, 변경 (선택적)
...
- 모든 디렉토리와 파일 명은 소문자와 ``_`` (Snake Case)로 기입한다.
- 디렉토리 안에는 ``__init__.py`` 파일이 있어야 한다. (파일 내용은 없음)
- 디렉토리 안에는 플러그인 타입에 맞는 **접두어 + '_' + 이름** 형식의 파이썬 모듈을 구현하여야 한다.
