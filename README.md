...
### 외부 프로세스 호출 흐름
<img src="images/process_flow.png" width="650" />

## 샘플 레시피

레시피(recipe)는 파이썬 신택스로 처리 흐름 할 일을 명시한다. 샘플 레시피를 통해 `Swak`의 사용법을 살펴보자. 입력과 출력 플러그인은 각각 `in` 및 `out` 패키지에서 참조한다.
...
테스트 모드에서는 하나의 테스크만 실행될 수 있다. 레시피 파일에 테스크가 하나 이상있다면, 실행할 테스크의 인덱스를 지정하자.
...
### 샘플 플러그인
간단한 샘플(`foo`) 플러그인을 예제로 하여 알아보자.

1. 먼저 GitHub에서 `swak-plugin-foo`라는 리포지토리를 만든다.
2. 리포지토리를 로컬로 `clone`한다.
  git clone https://githttps://github.com/haje01/swak-plugin-foo.git
3. 로컬에서 개발용으로 설치한다.
