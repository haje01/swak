...
Swak는 실행을 위해 홈 디렉토리를 필요로 한다. 이것은 다음과 같은 구조가 된다.
...
1. 스크립트나 실행 파일 인자로 디렉토리 경로를 받은 경우 그것을 이용.
2. `SWAK_HOME` 환경 변수가 설정되어 있으면 그것을 이용.
3. 스크립트(또는 실행파일)과 같은 디렉토리에 `config.yml`이 있으면 거기를 홈 디렉토리로 이용.
4. 에러를 발생한다.

경우에 따라 다음과 같은 식으로 홈 디렉토리를 지정한다.

- 개발을 위해 소스를 받은 경우는 소스 디렉토리 내의  `devhome`의 절대 경로를 실행 파일 인자 또는 환경 변수로 지정한다. 그러면 그 안의 개발용 `config.yml` 파일을 참고하여 실행된다.
- 본인 만의 커스텀한 설정이 필요한 경우, `devhome/config.yml`을 수정하지 말고, 별도로 디렉토리를 하나 만들고 그곳에 `config.yml`을 저장한 후, 홈 디렉토리로 지정해준다. (`logs/`, `run/` 폴더는 자동으로 만들어진다.`)
- 바이너리로 빌드되어 배포 될 때는, 일반적으로 실행 파일과 같은 폴더에 `config.yml`을 넣어주면 된다. 
