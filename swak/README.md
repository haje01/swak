...
### 데이터
Swak이 다루는 것은 **텍스트 데이터**이다. 만약 바이너리 데이터를 다루어야 한다면, 그것을 위한 플러그인을 통해 변환후 가능할 것이다. 내부적으로 Swak은 모든 텍스트를 **utf8**로 인코딩하여 사용한다.

### 라인
개행문자 `\n`으로 분리되는 텍스트 데이터를 **라인**이라 한다.

### 레코드
라인을 의미 단위로 파싱한 결과물을 **레코드**라고 한다. 레코드는 '키': '밸류' 형식, 즉 파이썬의 Dictonary 형식이다.

### 이벤트
레코드에 발생 시간 필드가 추가된 것을 이벤트 또는 데이터 이벤트라고 한다.
...
> 룰(Rule)은 패턴(Pattern)과 컬렉터(Collector)로 구성된다. 정확히는 패턴은 태그의 패턴을 말하며, 컬렉터는 출력 플러그인 또는 또 다른 이벤트 라우터를 말한다.

이벤트 라우터는 다음과 같이 동작한다:
1. 플러그인에서 emit 메소드를 호출하면 그 이벤트를 받아서
1. 이벤트의 태그와 일치하는 룰을 찾은 뒤
1. 이벤트를 룰의 컬렉터로 보낸다.
...
> 2.7을 지원하는 이유는 일부 OS의존 적인 외부 패키지의 활용을 위해서이고, 3.5를 지원하는 이유는 현재 PyInstaller가 지원하는 가장 높은 버전이기 때문이다.
