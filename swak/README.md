...
Swak의 입력 플러그인에서 읽어들인 라인들은 파서에 의해서 파싱이 된다. 파싱된 결과물을 **레코드**라 한다. 레코드는 '키': '밸류' 형식, 즉 `dict` 형식이다.
...
> 입력과 출력 플러그인은 각각 시작과 끝에 처리되지만, 나머지 종류의 플러그인 들은 꼭 이 순서대로 처리되는 것은 아니다. 새로운 태그 지정을 통해 다양한 순서로 처리될 수 있다.
